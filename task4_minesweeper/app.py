...
def sql_rescue_dialog(conn):
    ...
    if submit_clicked:
        ...
        if correct:
            ...
            # Do NOT call st.rerun() here — inside @st.dialog, st.rerun() triggers a
            # full app rerun which the main page misreads as "user closed dialog" → game over.
            # The dialog re-executes automatically after the button click (fragment rerun).
...
""", unsafe_allow_html=True)
        ...
        with c_restart:
            if st.button("\U0001F501 Restart", type="primary", key="ms_restart"):
                ss.ms_show_restart_picker = True
                st.rerun()
        with c_back:
            if st.button("\u2B05 Back", type="secondary", key="ms_back"):
